...
var hideItem = {
	...
	onCreate: function( id ) {
		...
		// to gigure out which item was actually hidden. It did cross my mind
		// I could color code them, but leaivng it as it is, is less work and
		// probably makes more sense to users without having to read a manual.
		
		// $('[data-panel="treeview"] [data-item-id="'+ itemData.id  +'"]').closest('.sdb-hidden').length > 0
		if ( isHidden ) {
			$('#dialog [data-item-id="'+ id +'"]').addClass( hideItem.hiddenClass );
