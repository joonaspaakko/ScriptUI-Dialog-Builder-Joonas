...
function numberInputs(){function r(e,t,i,a){var n=e.val();if(isNaN(n))e.val(g);else{n=parseInt(n,10);var d=$("#dialog .active"),r=e.hasClass("width"),l=e.hasClass("height"),o={min:parseInt(e.attr("min"),10),max:parseInt(e.attr("max"),10),step:a||parseInt(e.attr("step"),10),modifierStep:parseInt(e.attr("modifier-step"),10)},s=(i?o.modifierStep:o.step)||1,c=n<o.min,p=n>o.max,m=c||p;
// If value is 0 when the action starts → fetch
// visual size and then continue as normal...
if(v)if(v=!1,r)n=Math.round(d.width())+("down"===t?1:0)-("up"===t?1:0);else if(l){var u="Dialog"===$("#dialog .active").data("item-type")?d.find("> .padding-box").outerHeight():d.height();n=Math.round(u)+("down"===t?1:0)-("up"===t?1:0)}switch(t){case"blur":if(m){var f=c&&o.min||p&&o.max;e.val(f)}break;case"up":n<o.max&&(n=n+s>o.max?o.max:n+s,e.val(n),g=n);break;case"down":n>o.min&&(n=n-s<o.min?o.min:n-s,e.val(n),g=n);break}}item.funnel.update(e.data("edit"))}$(".number").wrap('<div class="number-wrap">');var g,v,e=$(".number-wrap");$('<div class="arrow plus"></div><div class="arrow minus"></div><div class="number-overlay"></div>').appendTo(e);var t=e.find(".number"),i=e.find(".number-overlay");t.each(function(){"none"===$(this).css("display")&&$(this).closest(".number-wrap").addClass("hide")});var l,o,s=!1,c=0,p=null,m=null;$(window).on("mousedown mousemove mouseup",function(e){if("mousedown"===e.type)s=!0,l=$(e.target).parent(),o=l.find("> .number"),g=o.val(),v=0==g;else if("mousemove"===e.type){if(s&&mousemovePing&&o.hasClass("number")){
...
p=t,m=e.screenY,e.pageY<c?r(o,"up",e.shiftKey,d):e.pageY>c&&r(o,"down",e.shiftKey,d),c=e.pageY}}else if("mouseup"===e.type){
...
i.on("click",function(){$(this).parent().find(".number").focus()}),t.on("focus blur",function(e){var t=$(this);"focus"===e.type?(g=t.val(),v=0==g,$(this).select()):r(t,"blur")}),e.find(".arrow").on("click",function(e){var t=$(this),i=t.parent().find(".number");g=i.val(),v=0==g,t.hasClass("plus")?r(i,"up",e.shiftKey):t.hasClass("minus")&&r(i,"down",e.shiftKey)}),t.on("keyup",function(e){var t=e.keyCode?e.keyCode:e.which,i=$(this);g=i.val(),v=0==g,// Arrow Up
r(i,38===t?"up":40===t?"down":"numberEntry",e.shiftKey)}),i.on("wheel",function(e){
...
var t=e.originalEvent.deltaY,i=e.originalEvent.deltaX,a=t<0||i<0,n=$(this).parent().find(".number");g=n.val(),v=0==g,// Scroll Up
r(n,a?"up":"down",e.shiftKey)})}function exportCode(){var e=local_storage.get("dialog");return"/* \nCode for Import https://scriptui.joonas.me — (Triple click to select): \n"+JSON.stringify(e)+"\n*/ \n\n"+getJSX(e)+"dialog.show();"}function clipBoardEvent(n){
/*global ClipboardJS*/
...
a=e;break;default:a=e+i[e]}return a}function makeJSXitem(e,t,i,a,n,d,r,l,o,s,c,p){var m=[!1],u="",f=n.toLowerCase();++i[f];var g=customVarNames(f,l,i);a[d]=g;var v=a[r];
// If current item is a parent...
"TreeItem"!==n&&(item.list[n.toLowerCase()](!1).parent?(u+="// "+g.toUpperCase()+"\n",u+="// "+Array(g.length+1).join("=")+"\n"):s.parent!==v&&s.name!==v&&(u+="// "+v.toUpperCase()+"\n",u+="// "+Array(v.length+1).join("=")+"\n"));var h=$("#dialog");
// This is where each item is first added
switch(n){case"Dialog":u+="var "+g+' = new Window("'+f+'"); \n';break;case"ListBox":case"DropDownList":var y=o.listItems.split("\n").join("").split(",");$.each(y,function(e){y[e]=y[e].trim()}),u+="var "+g+'_array = ["'+y.join('","')+'"]; \n',u+="var "+g+" = "+a[r]+'.add("'+f+'", undefined, '+g+"_array",void 0!==o.selection&&"ListBox"===n&&1<o.selection.length&&(u+=", {multiselect: true}"),u+="); \n";break;case"Divider":u+="var "+g+" = "+a[r]+'.add("panel"); \n';break;case"TreeView":var b=h.find('[data-item-id="'+d+'"]'),w=0<o.preferredSize[0]?o.preferredSize[0]:Math.round(b.width()),C=0<o.preferredSize[1]?o.preferredSize[1]:Math.round(b.height());u+="var "+g+" = "+a[r]+'.add("'+n.toLowerCase()+'", [0,0,'+(w+0)+","+(C+0)+"]); \n";break;case"TreeItem":var x=h.find('[data-item-id="'+d+'"]').hasClass("tree-node")?"node":"item";u+="var "+g+" = "+a[r]+'.add("'+x+'", "'+o.text+'"); \n';break;case"StaticText":
...
// ADD PARENT GROUP
// ALIGNMENT
if(u+="var "+g+" = "+a[r]+'.add("group"); \n',
// PREFERRED SIZE
void 0!==o.preferredSize&&(0<o.preferredSize[0]&&(u+=I+g+".preferredSize.width = "+o.preferredSize[0]+"; \n"),0<o.preferredSize[1]&&(u+=I+g+".preferredSize.height = "+o.preferredSize[1]+"; \n")),
// ORIENTATION
u+=I+g+'.orientation = "column"; \n',
// ALIGN CHILDREN
void 0!==o.justify&&(u+=I+g+'.alignChildren = ["'+o.justify+'","center"]; \n'),
// SPACING
u+=I+g+".spacing = 0; \n",null!=o.alignment){var S=t.items["item-"+r].style.orientation,k="";k="column"===S?("top"===o.alignment?"left":"bottom"===o.alignment&&"right")||o.alignment:("left"===o.alignment?"top":"right"===o.alignment&&"bottom")||o.alignment;var T=t.items["item-"+r].style.alignChildren;k="column"===S?'["'+k+'","'+T[1]+'"]':'["'+T[0]+'","'+k+'"]',u+=I+g+".alignment = "+k+"; \n"}
// SPACER
u+="\n";
...
u+=I+g+'.add("statictext", undefined, "'+t+'"); \n'})}else u+="var "+g+" = "+a[r]+'.add("'+f+'"); \n';break;case"EditText":m=multilineCheck(d);var P=$('#dialog [data-item-id="'+d+'"]'),_=Math.round(P.width()),D=Math.round(P.height()),j=m[0]?", [0,0, "+_+", "+D+" ], undefined, {multiline: true}":"";u+="var "+g+" = "+a[r]+'.add("'+f+'"'+j+"); \n";break;case"Image":u+="var "+g+'_array = ["'+encodeURIComponent(atob(o.image[0].split(",")[1].replace(/=$/,"").replace(/=$/,"")))+'"]; \n',u+="var "+g+" = "+a[r]+'.add("image", undefined, File.decode('+g+"_array[0]) ); \n";break;default:u+="var "+g+" = "+a[r]+'.add("'+f+'"); \n'}s.name=g,s.parent=a[r];var A=/*type === 'TreeView' ||*/"TreeItem"!==n||p?"\n":"";u+=styleJSXitem(t,i,a,n,d,r,l,o,g,c,m)+A;
// Add in treeItem expanded properties if this is the last of treeItems in this group
var L=t.order[e+1];if(void 0!==L){var E=t.items["item-"+L],B="TreeItem"===n&&"TreeItem"!==E.type;B&&0<c.length?(u+="\n",$.each(c,function(e,t){u+=t}),u+="\n",c=[]):B&&(u+="\n")}else void 0===L&&"TreeItem"===n&&($.each(c,function(e,t){u+=t}),u+="\n",c=[]);return u}function multilineCheck(e){var d=!1,r=[],l=$('#dialog [data-item-id="'+e+'"] .text-container'),t=l.html(),i=t.replace(/<br>$/,"").split(" ");// Just to make super sure the dialog text stays the same...
...
if("TreeItem"===a){var u=e.items["item-"+n].expanded,f=$('#dialog [data-item-id="'+n+'"]').parentsUntil(".tree-view").filter(".tree-view-item"),g=!1;$.each(f,function(){$(this).hasClass("expanded")||(g=!0);// CLOSE THE FLOOD GATES!!! AAAAAAAAAAAAAAAAAAAA!!!
}),u&&!1===g&&s.push(m+o+".expanded = true; \n")}else if("Divider"===a)p+=m+o+'.alignment = "fill"; \n';else if("Slider"===a)p+=m+o+".minvalue = 0; \n",p+=m+o+".maxvalue = 100; \n",p+=m+o+".value = 50; \n";else{if(
// DROP LIST SELECTION
...
// TREE VIEW - ITEM
treeViewItem.onUpdate(i,a),forceSize.onUpdate(e,i,a),
...
"all"===e?$.each(i.style,function(e,t){n.property=e,n.value=t,item.update.set_values(n)}):item.update.set_values(n),
...
/*global droplist*/
/*eslint no-undef: ["error", { "typeof": true }] */
droplist.set.items(d,t,a):"ListBox"===i&&
/*global listbox*/
/*eslint no-undef: ["error", { "typeof": true }] */
...
// MARGINS
case"margins":var o=t[0],s=t[1],c=t[2],p=t[3],m="object"!=typeof t,u=m?t:o,f=m?t:s,g=m?t:c,v=m?t:p;"Dialog"===i?r.css({paddingTop:u<=6?1:u,paddingRight:f<=1?1:f,paddingBottom:g<=1?1:g,paddingLeft:v<=1?1:v}):"Panel"===i||"Tab"===i?r.css({paddingTop:u<=3?3:u,paddingRight:f<=3?3:f,paddingBottom:g<=1?1:g,paddingLeft:v<=3?3:v}):r.css({paddingTop:u,paddingRight:f,paddingBottom:g,paddingLeft:v});break;
// PREFERRED SIZE
case"preferredSize":var h=0==t[0]?"auto":t[0],y=0==t[1]?"auto":t[1]+("Dialog"===i?$("#dialog-title-bar").outerHeight():0);d.css({minWidth:h,minHeight:y}),
/*global dangerZone*/
/*eslint no-undef: ["error", { "typeof": true }] */
dangerZone.set(e,d,r),
/*global droplist*/
/*eslint no-undef: ["error", { "typeof": true }] */
droplist.set.size(d,t,a,i,h,y);break;
// ORIENTATION
case"orientation":d.removeClass(function(e,t){return(t.match(/(^|\s)orientation-\S+/g)||[]).join(" ")}).addClass("orientation-"+t);break;
// SPACING
case"spacing":var b=d.find("> .padding-box"),w="> .padding-box";b.find("> style.spacing").remove();// Get rid of the old one.
var C=0,x=e.data.items["item-"+n].parentId;if(!1!==x)"row"===e.data.items["item-"+x].style.orientation&&(C=2);t+=C,$('<style class="spacing">#dialog [data-item-id="'+d.data("item-id")+'"].orientation-row '+w+" > div {padding-left: "+t+'px;}\n#dialog [data-item-id="'+d.data("item-id")+'"].orientation-row '+w+' > div:first-of-type {padding-left: 0px;}\n#dialog [data-item-id="'+d.data("item-id")+'"].orientation-column '+w+" > div {padding-top: "+t+'px;}\n#dialog [data-item-id="'+d.data("item-id")+'"].orientation-column '+w+" > div:first-of-type {padding-top: 0px;}</style>").appendTo(b);break;
// ALIGN CHILDREN
...
// IMAGE
case"image":d.find("img").attr("src",t[0]);break;
// ALIGNMENT
case"alignment":if(d.removeClass(function(e,t){return(t.match(/(^|\s)alignment-\S+/g)||[]).join(" ")}),null!==t){var I="left"===t&&["left","top"]||"top"===t&&["left","top"]||"right"===t&&["right","bottom"]||"bottom"===t&&["right","bottom"]||[t,t];d.addClass("alignment-horizontal-"+I[0]),d.addClass("alignment-vertical-"+I[1])}break}};var tabbedPanel={onCreate:function(e){
...
if($.each(t.split(","),function(e,t){var i=t.trim(),a="-"==i?" horizontal-line":"",n=e===d.selection?" selected test":"";e===d.selection&&(l=!0),$('<div class="'+n+a+'">'+i+"</div>").appendTo(r)}),!l){var i=local_storage.get("dialog");i.items["item-"+i.activeId].style.selection=0,local_storage.set("dialog",i),r.find("> div:first").addClass("selected")}var a=e.find(".drop-list-wrap");droplist.backbone(a)},size:function(e,t,i,a,n){
// Special treatment for Dropdownlist
if("DropDownList"===a){var d=e.find(".drop-list-wrap"),r=e.find("label");e.removeClass("too-big"),e.removeClass("too-small"),e.addClass("get-width");var l=e.width(),o=r.outerWidth(!0),s=d.outerWidth(!0);e.removeClass("get-width");var c=o+s;c<n?(e.addClass("too-big"),s<l&&d.width("auto")):n<c&&(e.addClass("too-small"),l<s&&d.width(l-16),e.parent().parent().hasClass("orientation-row")&&
// In this situation the label has position: absolute; so it doesn't respect the padding on the left side.
e.find("label").css({marginLeft:e.css("padding-left")}))}}},hide:function(){$("#drop-list").remove(),$(".drop-list-wrap.open").removeClass("open")}},radiocheck={init:function(e,d,t){var r=this,i=e.find(".radiocheck");r.restore(e,d,t,i),i.on("click",function(){var e=$(this),t=local_storage.get("dialog"),i=t.items["item-"+d],a=e.hasClass("on");e[a?"removeClass":"addClass"]("on");var n=!a;
// RADIOBUTTON
...
var d=t[0],r=t[1];i.find('#align-children-horizontal option:contains("'+d+'")').prop("selected",!0),i.find('#align-children-vertical option:contains("'+r+'")').prop("selected",!0);break;case"image":i=$('<div class="image-edit"><div class="img-wrapper"><img class="base64-bin" src="'+t[0]+'" alt="" /></div><div class="custom-file-input"><i class="fas fa-upload"></i>Choose file...</div><input data-edit="image" type="file" accept="image/jpeg, image/png"></div>');break;case"alignment":var l,o=$("#dialog .active").parent("div").parent("div").hasClass("orientation-column"),s=o?"column":"row",c=o?"horizontal":"vertical",p=null===t,m=o?["left","center","right","fill"]:["top","center","bottom","fill"],u="";$.each(m,function(e,t){u+='<option value="'+t+'">'+t+"</option>"}),i=$('<div class="alignment-container"><h2 title="The alignment style for this element. If defined, this value overrides the alignChildren setting for the parent container.">Alignment <span>(Self)</span></h2><div class="alignment-checkbox"><input type="checkbox" id="alignment-checkbox-input" name="" '+(p?"":"checked")+' /><label for="alignment-checkbox-input"></label></div><br>'+('<div id="alignment-'+c+'"><select name="qty" class="pretty-classic" data-edit="alignment"  data-edit-value="'+c+'" '+(p?"disabled":"")+" >"+u+"</select></div>")+"</div>"),"column"===s?(l=("top"===t?"left":"bottom"===t&&"right")||t,i.find('#alignment-horizontal option:contains("'+(null===t?"center":l)+'")').prop("selected",!0)):(l=("left"===t?"top":"right"===t&&"bottom")||t,i.find('#alignment-vertical option:contains("'+(null===t?"center":l)+'")').prop("selected",!0));break}return i}},propsPanel=$("#panel-edit-style-wrap");
...
// id         = dataItem.id,
i=e.dataItem.type,a=e.value,n=a[0],d=a[1],r=0!==n,l=0!==d,o=t,s=Math.round(o.width()),c=Math.round(o.height())-("Dialog"===i?Math.round($("#dialog-title-bar").outerHeight()):0),p=$("#panel-edit-style-wrap"),m=p.find(".number.width"),u=p.find(".number.height"),f="danger-zone";r&&!m.hasClass(f)&&n<s?m.addClass(f):(m.hasClass(f)&&!r||m.hasClass(f)&&s<=n)&&m.removeClass(f),l&&!u.hasClass(f)&&d<c?u.addClass(f):(u.hasClass(f)&&!l||u.hasClass(f)&&c<=d)&&u.removeClass(f)}},forceSize={onUpdate:function(e,p){$("#dialog").find([".static-text",".edit-text",".list-box",".tree-view"].join(", ")).each(function(){var e=$(this),t=e.data("item-id"),i=e.data("item-parent-id"),a={active:p.items["item-"+t],parent:p.items["item-"+i]},n="fill"!==a.parent.style.alignChildren[0],d="fill"!==a.parent.style.alignChildren[1],r=a.active.style.preferredSize[0],l=a.active.style.preferredSize[1],o=0!==r,s=0!==l,c="fill"!==a.active.style.alignment;o&&n&&c?e.width(r):"auto"!==e.width()&&e.width("auto"),s&&d&&c?e.height(l):"auto"!==e.height()&&e.height("auto")})}};
// CORE
// ****
...
var i=$(this).closest("[data-item-type]").hasClass("static-text"),a=$(this).closest("[data-item-type]").hasClass("edit-text");if(i||a){var n=$(this).height(),d=!1;i&&25.5<n?d=!0:a&&22.5<n&&(d=!0),d?$(this).addClass("multiline"):$(this).removeClass("multiline")}
