...
function numberInputs(){function d(e,t,i,a){var n=e.val();if(isNaN(n))e.val(p);else{n=parseInt(n,10);
...
// }
var r={min:parseInt(e.attr("min"),10),max:parseInt(e.attr("max"),10),step:a||parseInt(e.attr("step"),10),modifierStep:parseInt(e.attr("modifier-step"),10)},d=(i?r.modifierStep:r.step)||1,l=n<r.min,o=n>r.max,s=l||o;switch(t){case"blur":if(s){var c=l&&r.min||o&&r.max;e.val(c)}break;case"up":n<r.max&&(n=n+d>r.max?r.max:n+d,e.val(n),p=n);break;case"down":n>r.min&&(n=n-d<r.min?r.min:n-d,e.val(n),p=n);break}}item.funnel.update(e.data("edit"))}$(".number").wrap('<div class="number-wrap">');var p,e=$(".number-wrap");$('<div class="arrow plus"></div><div class="arrow minus"></div><div class="number-overlay"></div>').appendTo(e);var t=e.find(".number"),i=e.find(".number-overlay");t.each(function(){"none"===$(this).css("display")&&$(this).closest(".number-wrap").addClass("hide")});var l,o,s=!1,c=0,m=null,f=null;$(window).on("mousedown mousemove mouseup",function(e){if("mousedown"===e.type)s=!0,l=$(e.target).parent(),o=l.find("> .number");else if("mousemove"===e.type){if(s&&mousemovePing&&o.hasClass("number")){
// if ( dragStartElement.data('edit') === 'margins' ) {
// 	$('body').addClass('dragging-margins');
// }
// dragWrapper.find('.number-overlay').css({ position: 'fixed' });
if(mousemovePing=!1,e.preventDefault(),null===m)return m=Date.now(),void(f=e.screenY);var t=Date.now(),i=t-m,a=e.screenY-f,n=Math.abs(Math.round(a/i*(e.shiftKey?200:40))),r=4<n?n:0;// Poor mans slow start...
m=t,f=e.screenY,e.pageY<c?d(o,"up",e.shiftKey,r):e.pageY>c&&d(o,"down",e.shiftKey,r),c=e.pageY}}else if("mouseup"===e.type){
...
$(this).select()):d(t,"blur")}),e.find(".arrow").on("click",function(e){var t=$(this),i=t.parent().find(".number");t.hasClass("plus")?d(i,"up",e.shiftKey):t.hasClass("minus")&&d(i,"down",e.shiftKey)}),t.on("keyup",function(e){var t=e.keyCode?e.keyCode:e.which,i=$(this);// Arrow Up
d(i,38===t?"up":40===t?"down":"numberEntry",e.shiftKey)})}function exportCode(){var e=local_storage.get("dialog");return"/* \nCode for Import https://scriptui.joonas.me — (Triple click to select): \n"+JSON.stringify(e)+"\n*/ \n\n"+getJSX(e)+"dialog.show();"}function clipBoardEvent(n){
/*global ClipboardJS*/
...
$("#panel-new-item-wrap ul li").each(function(){f[$(this).data("item-type").toLowerCase()]=0});var e=$("#panel-tree-view-wrap .tree-dialog li"),v=e.length;return e.each(function(e){var t=e,i=$(this).data().itemId,a=c.items["item-"+i],n=a.parentId,r=0===n||!1===n?"Dialog":c.items["item-"+n].type,d=a.type,l=a.id,o=a.style,s=!1;e===v-1&&(s=!0),p+=makeJSXitem(t,c,f,m,d,l,n,r,o,u,g,s)}),p}
...
a=e;break;default:a=e+i[e]}return a}function makeJSXitem(e,t,i,a,n,r,d,l,o,s,c,p){var m=[!1],f="",u=n.toLowerCase();++i[u];var g=customVarNames(u,l,i);a[r]=g;var v=a[d];
...
switch(n){case"Dialog":f+="var "+g+' = new Window("'+u+'"); \n';break;case"ListBox":case"DropDownList":var y=o.listItems.split("\n").join("").split(",");$.each(y,function(e){y[e]=y[e].trim()}),f+="var "+g+'_array = ["'+y.join('","')+'"]; \n',f+="var "+g+" = "+a[d]+'.add("'+u+'", undefined, '+g+"_array",void 0!==o.selection&&"ListBox"===n&&1<o.selection.length&&(f+=", {multiselect: true}"),f+="); \n";break;case"Divider":f+="var "+g+" = "+a[d]+'.add("panel"); \n';break;case"TreeView":var b=h.find('[data-item-id="'+r+'"]'),w=0<o.preferredSize[0]?o.preferredSize[0]:Math.round(b.width()),x=0<o.preferredSize[1]?o.preferredSize[1]:Math.round(b.height());f+="var "+g+" = "+a[d]+'.add("'+n.toLowerCase()+'", [0,0,'+(w+0)+","+(x+0)+"]); \n";break;case"TreeItem":var C=h.find('[data-item-id="'+r+'"]').hasClass("tree-node")?"node":"item";f+="var "+g+" = "+a[d]+'.add("'+C+'", "'+o.text+'"); \n';break;case"StaticText":
...
if((m=multilineCheck(r))[0]){var I="    ";
// ADD PARENT GROUP
// ALIGNMENT
if(f+="var "+g+" = "+a[d]+'.add("group"); \n',
// PREFERRED SIZE
...
// SPACING
f+=I+g+".spacing = 0; \n",null!=o.alignment){var S=t.items["item-"+d].style.orientation,k="";k="column"===S?("top"===o.alignment?"left":"bottom"===o.alignment&&"right")||o.alignment:("left"===o.alignment?"top":"right"===o.alignment&&"bottom")||o.alignment;var T=t.items["item-"+d].style.alignChildren;k="column"===S?'["'+k+'","'+T[1]+'"]':'["'+T[0]+'","'+k+'"]',f+=I+g+".alignment = "+k+"; \n"}
// SPACER
f+="\n";
...
f+=I+g+'.add("statictext", undefined, "'+t+'"); \n'})}else f+="var "+g+" = "+a[d]+'.add("'+u+'"); \n';break;case"EditText":m=multilineCheck(r);var P=$('#dialog [data-item-id="'+r+'"]'),_=Math.round(P.width()),D=Math.round(P.height()),j=m[0]?", [0,0, "+_+", "+D+" ], undefined, {multiline: true}":"";f+="var "+g+" = "+a[d]+'.add("'+u+'"'+j+"); \n";break;case"Image":f+="var "+g+'_array = ["'+encodeURIComponent(atob(o.image[0].split(",")[1].replace(/=$/,"").replace(/=$/,"")))+'"]; \n',f+="var "+g+" = "+a[d]+'.add("image", undefined, File.decode('+g+"_array[0]) ); \n";break;default:f+="var "+g+" = "+a[d]+'.add("'+u+'"); \n'}s.name=g,s.parent=a[d];var A=/*type === 'TreeView' ||*/"TreeItem"!==n||p?"\n":"";f+=styleJSXitem(t,i,a,n,r,d,l,o,g,c,m)+A;
// Add in treeItem expanded properties if this is the last of treeItems in this group
var L=t.order[e+1];if(void 0!==L){var B=t.items["item-"+L],E="TreeItem"===n&&"TreeItem"!==B.type;E&&0<c.length?(f+="\n",$.each(c,function(e,t){f+=t}),f+="\n",c=[]):E&&(f+="\n")}else void 0===L&&"TreeItem"===n&&($.each(c,function(e,t){f+=t}),f+="\n",c=[]);return f}function multilineCheck(e){var r=!1,d=[],l=$('#dialog [data-item-id="'+e+'"] .text-container'),t=l.html(),i=t.replace(/<br>$/,"").split(" ");return l.html(""),$.each(i,function(e,t){var a=t.split("<br>");if(1<a.length)r=!0,$.each(a,function(e,t){var i=e===a.length-1;l.html(l.html()+(i?"":" ")+t+(i?"":"<br>")),d.push((i?"":" ")+t+(i?"":"<br>"))});else{var i=l.height(),n=0===e?"":" ";l.html(l.html()+n+t),
// New line has appeared.
// Joonas uses crushing depression. It's super effective!
i<l.height()?(d.push(t),r=!0,d.splice(d.length-1,0,"<br>")):d.push(n+t)}}),l.html(t),// Just to make super sure the dialog text stays the same...
console.log(d.join("")),[r,d.join("")]}function styleJSXitem(e,t,i,a,n,r,d,l,o,s,c){var p="",m="    ";
...
// TEXT
if("Tab"===a&&e.items["item-"+r].style.selection===n&&(p+=m+i[r]+".selection = "+o+"; \n"),
...
// HTML
var n=$('<div class="notification '+e+' animated"><div><i class="fas fa-info-circle icon"></i><div class="msg">'+t+"<div></div></div>").appendTo("#notifications-wrap"),r=$("#notifications-wrap .notification").length;
// Crowd control - GET BACK YOU MONSTERS!!!! *hoses the crap out of the crowd while frantically digging for the pepper spray*
1<=r&&a($("#notifications-wrap .notification").slice(0,r-1));var d=n.height();n.css({height:0,visibility:"visible"}),n.animate({height:d},300,"easeInOutBack"),
...
var d=local_storage.get("dialog");edit_style_panel.build(d.items["item-"+e].style)}$("#dialog-section").backstretch("resize")},update:function(e,t){droplist.hide();
// LOCAL STORAGE
...
if(i&&(n=t.items.hasOwnProperty(a)),t.activeId=e.id,!n){void 0===t.items&&(t.items={}),void 0===t.items[a]&&(t.items[a]={});var r=t.items[a];
// Add item id
r.id=e.id,
// Add item type
r.type=e.type,
// Add parent id
r.parentId=e.parentId,
// Copy style from existing element
// SourceId is used in item duplication. If it's set, duplication is in process.
e.sourceId?r.style=$.extend(!0,{},t.items["item-"+e.sourceId].style):r.style=item.list[e.type.toLowerCase()](e).defaultStyle}
...
var n,r=e.event.match(/^drag/);
// Append to parent items
n=t?r?e.previousIsParent?"insertAfter":"prependTo":"appendTo":"insertAfter",$(i)[n](e.target)},dialogPreview:function(e){var t,i,a=item.list[e.type.toLowerCase()](e).previewHtml,n=e.target.is("ul")?e.target.parent("li"):e.target,r=n.data("parent"),d=n.data("item-id"),l=$("#dialog"),o=l.find('[data-item-id="'+d+'"]'),s=e.event.match(/^drag/);
// No data in ul, so gotta go a step higher to look for the data...
// Append to parent items
i=r?(t=s?e.previousIsParent?"insertAfter":"prependTo":"appendTo","Dialog"===e.type?l:s&&e.previousIsParent?o:o.find("> .padding-box")):(t="insertAfter",l.find('[data-item-id="'+d+'"]')),$(a)[t](i);var c=$('#dialog [data-item-id="'+e.id+'"]');"DropDownList"===e.type?droplist.init(c,e.id):"RadioButton"===e.type||"Checkbox"===e.type?radiocheck.init(c,e.id,e.type):"ListBox"===e.type&&listbox.init(c,e.id)}},item.get={},item.get.order=function(){var t=[];return $("#panel-tree-view-wrap .contents [data-item-id]").each(function(){var e=$(this).data("item-id");t.push(e)}),t},item.get.id=function(){var e=item.get.order();return Math.max.apply(null,e)+1},item.activate=function(e){droplist.hide();
...
// Write back to local storage...
local_storage.set("dialog",a)},dialogPreview:function(e,t,i,a,n){var r=$("#dialog"),d=r.find('[data-item-id="'+e+'"]'),l=r.find('[data-item-id="'+n+'"]'),o="insertAfter"===a?l:l.find("> .padding-box");
...
if(d[a](o),"RadioButton"===i){var s=d.find(".radiobutton"),c=s.hasClass("on"),p=0,m=':not([data-item-type="RadioButton"],.spacing)';if(d.nextUntil(m).add(d.prevUntil(m)).each(function(){$(this).find(".radiobutton").hasClass("on")&&++p}),0<p&&c){s.removeClass("on");var f=local_storage.get("dialog");f.items["item-"+e].style.checked=!1,local_storage.set("dialog",f)}}}},
...
},item.update.style.treeView=function(e,t,i){if("text"===e||"all"===e){var a=i.style.text,n=$('#panel-tree-view-wrap [data-item-id="'+i.id+'"] > .item-text'),r=i.type,d=void 0===a?r:a.trim();n.html(r.toLowerCase()===d.toLowerCase()?r:'<span class="type">'+r+':</span> <span class="txt">'+a+"</span>")}},item.update.style.dialogPreview=function(e,t,i,a){var n={property:e,value:i.style[e],data:t,dataItem:i,event:a};
...
// - single number value
if(0<i.find(".n-3-4.hidden").length)t.margins=parseInt(i.find(".margin-inputs .top").val(),10);else{var n=[];i.find('[data-edit="margins"]').each(function(){n.push(parseInt($(this).val(),10))}),t.margins=n}break;case"preferredSize":var r=parseInt(i.find("input.width").val(),10),d=parseInt(i.find("input.height").val(),10);t.preferredSize=[r,d];break;case"orientation":t.orientation=a.find("option:selected").val();break;case"spacing":t.spacing=parseInt(a.val(),10);break;case"alignChildren":var l=i.find('#align-children-horizontal[data-edit="alignChildren"] option:selected').val(),o=i.find('#align-children-vertical[data-edit="alignChildren"] option:selected').val();t.alignChildren=[l,o];break;case"image":t.image=[i.find(".base64-bin").attr("src")];break;case"alignment":!1===a.prop("disabled")?t.alignment=a.find("option:selected").val():t.alignment=null;break}
// console.table( data );
return t},
// Updates the Dialog preview look
item.update.set_values=function(e){var t=e.value,i=e.dataItem.type,a=e.dataItem.style,n=e.dataItem.id,r=$("#dialog .active"),d=r.find("> .padding-box");switch(e.property){
// TEXT
case"text":if("Dialog"===i)r.find("#dialog-title-bar div").text(t);else if("Panel"===i){var l=r.find("> .title");l.text(t),
// Makes sure the panel container is always wide enough to cover the title
d.css({minWidth:l.width()+22})}
// type === 'RadioButton' || type === 'Checkbox' || type === 'DropDownList'
else if(0<r.find("> label").length)r.find("label").text(t);else if("Tab"===i)r.parent().parent().find('> .tab-container [data-tab-id="'+n+'"]').text(t);else if("TreeItem"===i)r.find("> .item-wrap .text-container").text(t);else if(item.list[i.toLowerCase()](!1).multiline){r.find(".text-container").html(t.split("\n").join("<br>"))}else r.find(".text-container").html(t);break;
// LIST ITEMS
case"listItems":"DropDownList"===i?
/*global droplist*/
/*eslint no-undef: ["error", { "typeof": true }] */
droplist.set(r,t,a):"ListBox"===i&&
/*global listbox*/
/*eslint no-undef: ["error", { "typeof": true }] */
listbox.set(r,t,a);break;
// CHECKED
case"checked":!0===t&&r.find("input").prop("checked",!0);break;
// JUSTIFY
case"justify":r.removeClass(function(e,t){return(t.match(/(^|\s)justify-\S+/g)||[]).join(" ")}).addClass("justify-"+e.value);break;
// MARGINS
case"margins":var o=t[0],s=t[1],c=t[2],p=t[3],m="object"!=typeof t,f=m?t:o,u=m?t:s,g=m?t:c,v=m?t:p;"Dialog"===i?d.css({paddingTop:f<=6?1:f,paddingRight:u<=1?1:u,paddingBottom:g<=1?1:g,paddingLeft:v<=1?1:v}):"Panel"===i||"Tab"===i?d.css({paddingTop:f<=3?3:f,paddingRight:u<=3?3:u,paddingBottom:g<=1?1:g,paddingLeft:v<=3?3:v}):d.css({paddingTop:f,paddingRight:u,paddingBottom:g,paddingLeft:v});break;
// PREFERRED SIZE
case"preferredSize":
...
if(r.css({width:h,height:y+(0!=t[1]&&"Dialog"===i?23:"")}),"DropDownList"===i){var b=r.find(".drop-list-wrap"),w=r.find("label");r.removeClass("too-big"),r.removeClass("too-small"),r.addClass("get-width");var x=r.width(),C=w.outerWidth(!0),I=b.outerWidth(!0);r.removeClass("get-width");var S=C+I;S<h?(r.addClass("too-big"),I<x&&b.width("auto")):h<S&&(r.addClass("too-small"),x<I&&b.width(x-16),r.parent().parent().hasClass("orientation-row")&&
// In this situation the label has position: absolute; so it doesn't respect the padding on the left side.
r.find("label").css({marginLeft:r.css("padding-left")}))}break;
// ORIENTATION
case"orientation":r.removeClass(function(e,t){return(t.match(/(^|\s)orientation-\S+/g)||[]).join(" ")}).addClass("orientation-"+t);break;
// SPACING
case"spacing":var k=r.find("> .padding-box"),T="> .padding-box";k.find("> style.spacing").remove();// Get rid of the old one.
var z=0,P=e.data.items["item-"+n].parentId;if(!1!==P)"row"===e.data.items["item-"+P].style.orientation&&(z=2);t+=z,$('<style class="spacing">#dialog [data-item-id="'+r.data("item-id")+'"].orientation-row '+T+" > div {padding-left: "+t+'px;}\n#dialog [data-item-id="'+r.data("item-id")+'"].orientation-row '+T+' > div:first-of-type {padding-left: 0px;}\n#dialog [data-item-id="'+r.data("item-id")+'"].orientation-column '+T+" > div {padding-top: "+t+'px;}\n#dialog [data-item-id="'+r.data("item-id")+'"].orientation-column '+T+" > div:first-of-type {padding-top: 0px;}</style>").appendTo(k);break;
// ALIGN CHILDREN
case"alignChildren":r.removeClass(function(e,t){return(t.match(/(^|\s)align-children-\S+/g)||[]).join(" ")}),r.addClass("align-children-horizontal-"+t[0]),r.addClass("align-children-vertical-"+t[1]);break;
// IMAGE
case"image":r.find("img").attr("src",t[0]);break;
// ALIGNMENT
case"alignment":if(r.removeClass(function(e,t){return(t.match(/(^|\s)alignment-\S+/g)||[]).join(" ")}),null!==t){var _="left"===t&&["left","top"]||"top"===t&&["left","top"]||"right"===t&&["right","bottom"]||"bottom"===t&&["right","bottom"]||[t,t];r.addClass("alignment-horizontal-"+_[0]),r.addClass("alignment-vertical-"+_[1])}break}};var tabbedPanel={onCreate:function(e){
...
var r=t.find(".active").data("item-id");item.activate(a),n.id=item.get.id(),item.funnel.create(n),// Filler tab N.2.
item.activate(r)}
...
var d=$("#panel-new-item-wrap .tab");d.hasClass("show")||d.addClass("show")},
...
onClick:function(e,t){var i=t.data("item-type"),a=e.hasClass("tab"),n="TabbedPanel"===i,r=a&&!n||!a&&n;
// ITEMS
if(r){var d=$("#panel-edit-style-wrap .edit-info");d.removeClass("highlight-animation"),// Reset so that the animation can run again if user tries to add another item with similar issues.
setTimeout(function(){d.addClass("highlight-animation")},10)}return r}},tab={onCreate:function(e){if("Tab"===e.type){var t=$('#dialog [data-item-id="'+e.parentId+'"]'),i=t.find("> .tab-container");
...
onSort:function(e){var t=e.hasClass("tab"),i=e.hasClass("tabbedpanel");if(t||i||0<$('#dialog [data-item-id="'+e.data("item-id")+'"]').closest(".tab").length){$("body").removeClass("dragging-tab");var a=local_storage.get("dialog"),n=e.data("item-id"),r=a.items["item-"+n].parentId,d=$("#dialog"),l=d.find('[data-item-id="'+r+'"]'),o=l.find("> .tab-container");d.find('.tab-container [data-tab-id="'+n+'"]').appendTo(o);
// Sort the shelf
var s=l.find("> .padding-box > .tab");tab.containerSort(s,o),item.activate(n),
// Build Item Properties panel
edit_style_panel.build(a.items["item-"+n].style),
// Update
tab.onUpdate.init(a,n)}},containerSort:function(e,t){e.each(function(){var e=$(this).data("item-id");t.find('[data-tab-id="'+e+'"]').appendTo(t)})},onUpdate:{init:function(d,e){
...
var t=$('#panel-tree-view-wrap [data-item-id="'+e+'"]');0<t.closest(".tabbedpanel").length&&t.parentsUntil(".dialog").filter(".tabbedpanel").each(function(){var e=d.items["item-"+$(this).data("item-id")],t=$('#dialog [data-item-id="'+e.id+'"]'),i=0,a=0;t.find("> .padding-box > .tab").each(function(){
...
var n=t.find("> .tab-container"),r=n.width()+2*parseInt(n.css("margin-left"),10);i<r&&(i=r),
...
var r=t.find(".active").data("item-id");item.activate(a),
...
var d=$("#panel-new-item-wrap .treeitem");d.hasClass("show")||d.addClass("show")},
// Prevents dragging items to specific containers...
// True = Prevent dropping
onDragValid:function(e,t){var i=t.target.parent("li"),a=e.hasClass("treeitem"),n=i.hasClass("treeview"),r=i.hasClass("treeitem");
// ITEMS
// True = Prevent dropping
...
onClick:function(e,t){var i=t.data("item-type"),a=e.hasClass("treeitem"),n="TreeView"===i,r="TreeItem"===i,d=a&&!n&&!r||!a&&(n||r);
// ITEMS
if(d){var l=$("#panel-edit-style-wrap .edit-info");l.removeClass("highlight-animation"),// Reset so that the animation can run again if user tries to add another item with similar issues.
setTimeout(function(){l.addClass("highlight-animation")},10)}return d}},treeViewItem={onClick:function(){$("#dialog").on("click",".tree-view-arrow",function(){var e=$(this).parent(".item-wrap").parent(".tree-view-item"),t=e.data("item-id"),i=local_storage.get("dialog"),a=i.items["item-"+t];treeViewItem.expand(i,a,e,!0)})},onSort:function(e,t,i){if("TreeItem"===t){$('#dialog [data-item-id="'+i+'"]').parent(".padding-box").parent(".tree-view-item").addClass("tree-node");
// Get rid of previous data
var a=$("#dialog .tree-node > .padding-box:empty").parent().removeClass("tree-node expanded");if(0<a.length){var n=a.data("item-id"),r=local_storage.get("dialog");delete r.items["item-"+n].expanded,local_storage.set("dialog",r)}}},onUpdate:function(e,t){if("TreeItem"===t.type){var i=$('#dialog [data-item-id="'+t.id+'"]');0<i.parent(".padding-box").parent(".tree-view-item").length&&i.parent(".padding-box").parent(".tree-view-item").addClass("tree-node"),treeViewItem.expand(e,t,i)}},expand:function(e,t,i,a){var n,r="expanded";if(!0!==a)n=t.expanded?"addClass":"removeClass";else{var d=i.hasClass(r);n=d?"removeClass":"addClass"}i[n](r),a&&(
// COLLAPSE
d?delete e.items["item-"+t.id].expanded:e.items["item-"+t.id].expanded=!0,local_storage.set("dialog",e))}};treeViewItem.onClick();var listbox={init:function(t,i){t.on("click","li",function(){var e=$(this);listbox.clickety(i,t,e)})},clickety:function(e,t,i){
// LIGHT SWITCH...
var a="selected",n=i.hasClass(a)?"removeClass":"addClass";i[n](a);
// GET ALL SELECTED ITEMS...
var r=[];t.find("li").each(function(){$(this).hasClass(a)&&r.push($(this).index())});
// WRITE SELECTION LOCAL STORAGE...
var d=local_storage.get("dialog");d.items["item-"+e].style.selection=r,local_storage.set("dialog",d)},set:function(e,t){var n=e.find("ul");n.children().remove();var i,a,r=t.split(","),d=local_storage.get("dialog"),l=d.items["item-"+d.activeId];
// Remove last selection if it's index is larger than the current list
void 0!==l.style.selection&&(i=l.style.selection,a=0,$.each(i,function(e,t){a<t&&(a=t)}),a>=r.length&&l.style.selection.pop(),local_storage.set("dialog",d)),
// Rebuild the list
$.each(r,function(e,t){var i=t.trim(),a=0<=$.inArray(e,l.style.selection);$("<li"+(a?' class="selected"':"")+"><span>"+i+"</span></li>").appendTo(n)})}};// This droplist has so many smoking mirrors... Practically all mirrors at this point... **FRAGILE**
...
if($.each(t.split(","),function(e,t){var i=t.trim(),a="-"==i?" horizontal-line":"",n=e===r.selection?" selected test":"";e===r.selection&&(l=!0),$('<div class="'+n+a+'">'+i+"</div>").appendTo(d)}),!l){var i=local_storage.get("dialog");i.items["item-"+i.activeId].style.selection=0,local_storage.set("dialog",i),d.find("> div:first").addClass("selected")}var a=e.find(".drop-list-wrap");droplist.backbone(a)},hide:function(){$("#drop-list").remove(),$(".drop-list-wrap.open").removeClass("open")}},radiocheck={init:function(e,r,t){var d=this,i=e.find(".radiocheck");d.restore(e,r,t,i),i.on("click",function(){var e=$(this),t=local_storage.get("dialog"),i=t.items["item-"+r],a=e.hasClass("on");e[a?"removeClass":"addClass"]("on");var n=!a;
// RADIOBUTTON
e.is(".radiobutton")&&d.radio.clearSiblings(t,e),i.style.checked=n,local_storage.set("dialog",t)})},restore:function(e,t,i,a){!0===local_storage.get("dialog").items["item-"+t].style.checked&&a.addClass("on")},radio:{clearSiblings:function(a,e){var t=e.parent(),i=':not([data-item-type="RadioButton"],.spacing)';
...
// Style number inputs
numberInputs();var r=item.list[i](!1).defaultStyle.text,d=item.list[i](!1).multiline,l=$('#panel-edit-style-wrap [data-edit="text"]'),o=l.parent();
// Icon on the text container telling the user they can't add linebreaks to current item
!r||d||o.hasClass("is-not-multiline")||o.addClass("is-not-multiline"),
// Style dropdowns
...
"dialog"!==t&&r&&(l.focus(),l.select())}var s=n.editInfo;s&&$('<div class="edit-info">'+s+"</div>").appendTo(a)}},panel_edit_style_html={
...
// IMPORT EVENT
$("#toolbar .import").on("click",function(){var r,d;modal.init('<div id="import-box"><h2>Import.jsx</h2><div class="ta"><div class="placeholder"><span class="important">The current dialog will be overwritten on a successful import.</span> <br><br>You can only import JSON generated by this this tool on export. 3rd line of every export includes the impotable JSON. Your old dialog code written in Javascript will <strong>not</strong> work! <br><br>Valid JSON is beautified (restructured) on paste <br> so that it is easier to read if necessary. <br></br><strong>Paste below</strong> <br><strong>↓</strong></div><div class="code"></div></div><div data-enter class="import-btn animated infinite"><i class="fas fa-arrow-right"></i><i class="fas fa-times"></i></div></div>'),(d=CodeMirror($("#import-box .code")[0],{mode:{name:"javascript",json:!0},theme:"monokai",autofocus:!0,lineNumbers:!0})).on("change",function(e,t){if("paste"===t.origin)
/*eslint no-empty: "error"*/
try{
// Poor man's 'beautify' on paste
var i=d.getValue();r=i;var a=JSON.parse(i),n=JSON.stringify(a,null,3);d.getDoc().setValue(n)}catch(e){
// continue regardless of error
}
// This section makes sure the #export-box doesn't spill past the viewport
setTimeout(function(){var e=$(window).height(),t=$("#import-box"),i=t.height();if(e<i+60){var a=$(window).height()-(i-t.find(".CodeMirror").height()),n=a-160,r=e<n?a-20:n;t.find(".code").css({maxHeight:r})}},10)}),$("#import-box").find(".import-btn").on("click",function(){
/*eslint no-empty: "error"*/
var e;try{var t=r.trim();e=JSON.parse(t)}catch(e){
// continue regardless of error
}
...
var t=e.parent("ul").parent("li").data("item-id"),i=e.prev(),a=0<i.length?"insertAfter":"prependTo",n=0<i.length?i.data("item-id"):t;e.attr("item-parent-id",t),e.data({"item-parent-id":t});var r=e.data("item-type"),d=e.data("item-id");item.funnel.sort(d,t,r,a,n),item.activate(d);
// Build Item Properties panel
var l=local_storage.get("dialog");edit_style_panel.build(l.items["item-"+d].style),tab.onSort(e),treeViewItem.onSort(e,r,d)},
// *********************************
// DRAG - MAKE NEW ITEM (onDrop)
// *********************************
item.drag.make=function(e){var t=e.prev(),i=e.parent("ul"),a=t.length<1,n=t.data("parent"),r=a?i:t,d={id:item.get.id(),// Picked up in the function onDragStart.
...
target:r,previousIsParent:n,event:"drag"};// Traget previous item. If it doesn't exist, target parent item.
e.remove(),// Item is the dragged li from the "Add Items" panel
item.funnel.create(d)},
// **************************************
// DRAG - DUPLICATE ITEM(S) (onDrop)
// **************************************
item.drag.duplicate=function(e,d){var t=e.prev(),i=e.parent("ul"),a=t.length<1,l=t.data("parent"),o=a?i:t;// Target previous item. If it doesn't exist, target parent item.
...
e.find("[data-item-id]").add(e).each(function(e){var t=$(this).data("item-id"),i=c.items["item-"+t],a=item.get.id(),n=0===e?d.target.parent("li").data("item-id"):m["parent-"+i.parentId];
// Maps old parent id with the new parent id so that
// following items can then check what they new parent id is
$(this).data("parent")&&(m["parent-"+t]=a);var r={id:a,type:i.type,parentId:n,target:0===e?o:p.find('[data-item-id="'+n+'"] > ul'),event:0===e?"drag-duplicate":"loadFromLocalStorage",previousIsParent:l,sourceId:t};0===e&&(s=r.id),item.funnel.create(r)}),
// Reactivate the ye olde active item
...
var i=$(this).closest("[data-item-type]").hasClass("static-text"),a=$(this).closest("[data-item-type]").hasClass("edit-text");if(i||a){var n=$(this).height(),r=!1;i&&25.5<n?r=!0:a&&22.5<n&&(r=!0),r?$(this).addClass("multiline"):$(this).removeClass("multiline")}
