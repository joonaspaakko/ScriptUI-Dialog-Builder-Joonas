...
u={name:"",parent:""},v=[];// JUST DO IT
// Creates rest of the counters based on the "Add items" panel...
$("#panel-new-item-wrap ul li").each(function(){f[$(this).data("item-type").toLowerCase()]=0});var e=$("#panel-tree-view-wrap .tree-dialog li"),g=e.length;return e.each(function(e){var t=e,i=$(this).data().itemId,a=c.items["item-"+i],n=a.parentId,d=0===n||!1===n?"Dialog":c.items["item-"+n].type,r=a.type,l=a.id,o=a.style,s=!1;e===g-1&&(s=!0),p+=makeJSXitem(t,c,f,m,r,l,n,d,o,u,v,s)}),p}
...
a=e;break;default:a=e+i[e]}return a}function makeJSXitem(e,t,i,a,n,d,r,l,o,s,c,p){var m=[!1],f="",u=n.toLowerCase();++i[u];var v=customVarNames(u,l,i);a[d]=v;var g=a[r];
// If current item is a parent...
"TreeItem"!==n&&(item.list[n.toLowerCase()](!1).parent?(f+="// "+v.toUpperCase()+"\n",f+="// "+Array(v.length+1).join("=")+"\n"):s.parent!==g&&s.name!==g&&(f+="// "+g.toUpperCase()+"\n",f+="// "+Array(g.length+1).join("=")+"\n"));var h=$("#dialog");
// This is where each item is first added
switch(n){case"Dialog":f+="var "+v+' = new Window("'+u+'"); \n';break;case"ListBox":case"DropDownList":var y=o.listItems.split("\n").join("").split(",");$.each(y,function(e){y[e]=y[e].trim()}),f+="var "+v+'_array = ["'+y.join('","')+'"]; \n',f+="var "+v+" = "+a[r]+'.add("'+u+'", undefined, '+v+"_array",void 0!==o.selection&&"ListBox"===n&&1<o.selection.length&&(f+=", {multiselect: true}"),f+="); \n";break;case"Divider":f+="var "+v+" = "+a[r]+'.add("panel"); \n';break;case"TreeView":var b=h.find('[data-item-id="'+d+'"]'),w=0<o.preferredSize[0]?o.preferredSize[0]:Math.round(b.width()),x=0<o.preferredSize[1]?o.preferredSize[1]:Math.round(b.height());f+="var "+v+" = "+a[r]+'.add("'+n.toLowerCase()+'", [0,0,'+(w+0)+","+(x+0)+"]); \n";break;case"TreeItem":var C=h.find('[data-item-id="'+d+'"]').hasClass("tree-node")?"node":"item";f+="var "+v+" = "+a[r]+'.add("'+C+'", "'+o.text+'"); \n';break;case"StaticText":
...
// ADD PARENT GROUP
// ALIGNMENT
if(f+="var "+v+" = "+a[r]+'.add("group"); \n',
// PREFERRED SIZE
void 0!==o.preferredSize&&(0<o.preferredSize[0]&&(f+=I+v+".preferredSize.width = "+o.preferredSize[0]+"; \n"),0<o.preferredSize[1]&&(f+=I+v+".preferredSize.height = "+o.preferredSize[1]+"; \n")),
// ORIENTATION
f+=I+v+'.orientation = "column"; \n',
// ALIGN CHILDREN
void 0!==o.justify&&(f+=I+v+'.alignChildren = ["'+o.justify+'","center"]; \n'),
// SPACING
f+=I+v+".spacing = 0; \n",null!=o.alignment){var S=t.items["item-"+r].style.orientation,k="";k="column"===S?("top"===o.alignment?"left":"bottom"===o.alignment&&"right")||o.alignment:("left"===o.alignment?"top":"right"===o.alignment&&"bottom")||o.alignment;var T=t.items["item-"+r].style.alignChildren;k="column"===S?'["'+k+'","'+T[1]+'"]':'["'+T[0]+'","'+k+'"]',f+=I+v+".alignment = "+k+"; \n"}
// SPACER
f+="\n";
...
f+=I+v+'.add("statictext", undefined, "'+t+'"); \n'})}else f+="var "+v+" = "+a[r]+'.add("'+u+'"); \n';break;case"EditText":m=multilineCheck(d);var P=$('#dialog [data-item-id="'+d+'"]'),_=Math.round(P.width()),D=Math.round(P.height()),j=m[0]?", [0,0, "+_+", "+D+" ], undefined, {multiline: true}":"";f+="var "+v+" = "+a[r]+'.add("'+u+'"'+j+"); \n";break;default:f+="var "+v+" = "+a[r]+'.add("'+u+'"); \n'}s.name=v,s.parent=a[r];var L=/*type === 'TreeView' ||*/"TreeItem"!==n||p?"\n":"";f+=styleJSXitem(t,i,a,n,d,r,l,o,v,c,m)+L;
...
if("TreeItem"===a){var f=e.items["item-"+n].expanded,u=$('#dialog [data-item-id="'+n+'"]').parentsUntil(".tree-view").filter(".tree-view-item"),v=!1;$.each(u,function(){$(this).hasClass("expanded")||(v=!0);// CLOSE THE FLOOD GATES!!! AAAAAAAAAAAAAAAAAAAA!!!
}),f&&!1===v&&s.push(m+o+".expanded = true; \n")}else if("Divider"===a)p+=m+o+'.alignment = "fill"; \n';else if("Slider"===a)p+=m+o+".minvalue = 0; \n",p+=m+o+".maxvalue = 100; \n",p+=m+o+".value = 50; \n";else{if(
// DROP LIST SELECTION
...
// CHECKED
// PREFERRED SIZE
if(!0===l.checked&&(p+=m+o+".value = "+l.checked+"; \n"),void 0!==l.preferredSize&&"TreeView"!==a){var g=l.preferredSize[0],h=l.preferredSize[1];c[0]||(0<g&&(p+=m+o+".preferredSize.width = "+g+"; \n"),0<h&&(p+=m+o+".preferredSize.height = "+h+"; \n"))}
// JUSTIFY
...
item.create.dialogPreview(e),item.activate(e.id),tab.onCreate(e);item.funnel.update("all","localStorage"),"loadFromLocalStorage"===e.event||edit_style_panel.build(e.style),"TabbedPanel"===e.type&&tabbedPanel.onCreate(e.event),"TreeView"===e.type&&treeView.onCreate(e.event);var t=$('#dialog [data-item-id="'+e.id+'"]');if("EditText"===e.type){var i,a=t.find(".edit-text-inner-wrap"),n=t.find("[contenteditable]"),d=$('<div class="et-double" style="height: 0; overflow: hidden;"></div>'),r=function(){d.html(n.html())};i=a.overlayScrollbars({paddingAbsolute:!0}).overlayScrollbars(),$(i.getElements().host).append(d),r(),n.on("input",function(){r()})}else"ListBox"===e.type&&t.find(".inner-wrap").overlayScrollbars({});$("#dialog-section").backstretch("resize")},remove:function(e){var t=$('#panel-tree-view-wrap [data-item-id="'+e+'"]'),i=t.prev(),a=t.next(),n=t.parent("ul").parent("li"),d=t.data("item-type");
...
// MARGINS
case"margins":var s=t[0],c=t[1],p=t[2],m=t[3],f="object"!=typeof t,u=f?t:s,v=f?t:c,g=f?t:p,h=f?t:m;"Dialog"===i?r.css({paddingTop:u<=6?1:u,paddingRight:v<=1?1:v,paddingBottom:g<=1?1:g,paddingLeft:h<=1?1:h}):"Panel"===i||"Tab"===i?r.css({paddingTop:u<=3?3:u,paddingRight:v<=3?3:v,paddingBottom:g<=1?1:g,paddingLeft:h<=3?3:h}):r.css({paddingTop:u,paddingRight:v,paddingBottom:g,paddingLeft:h});break;
// PREFERRED SIZE
case"preferredSize":
...
var y=0==t[0]?"auto":t[0],b=0==t[1]?"auto":t[1];
// Special treatment for Dropdownlist
if(d.css({width:y,height:b+("Dialog"===i&&23)}),"DropDownList"===i){var w=d.find(".drop-list-wrap"),x=d.find("label");d.removeClass("too-big"),d.removeClass("too-small"),d.addClass("get-width");var C=d.width(),I=x.outerWidth(!0),S=w.outerWidth(!0);d.removeClass("get-width");var k=I+S;k<y?(d.addClass("too-big"),S<C&&w.width("auto")):y<k&&(d.addClass("too-small"),C<S&&w.width(C-16),d.parent().parent().hasClass("orientation-row")&&
...
// SPACING
case"spacing":var T=d.find("> .padding-box"),z="> .padding-box";T.find("> style.spacing").remove();// Get rid of the old one.
var P=0,_=e.data.items["item-"+n].parentId;if(!1!==_)"row"===e.data.items["item-"+_].style.orientation&&(P=2);t+=P,$('<style class="spacing">#dialog [data-item-id="'+d.data("item-id")+'"].orientation-row '+z+" > div {padding-left: "+t+'px;}\n#dialog [data-item-id="'+d.data("item-id")+'"].orientation-row '+z+' > div:first-of-type {padding-left: 0px;}\n#dialog [data-item-id="'+d.data("item-id")+'"].orientation-column '+z+" > div {padding-top: "+t+'px;}\n#dialog [data-item-id="'+d.data("item-id")+'"].orientation-column '+z+" > div:first-of-type {padding-top: 0px;}</style>").appendTo(T);break;
// ALIGN CHILDREN
...
// ALIGNMENT
case"alignment":if(d.removeClass(function(e,t){return(t.match(/(^|\s)alignment-\S+/g)||[]).join(" ")}),null!==t){var D="left"===t&&["left","top"]||"top"===t&&["left","top"]||"right"===t&&["right","bottom"]||"bottom"===t&&["right","bottom"]||[t,t];d.addClass("alignment-horizontal-"+D[0]),d.addClass("alignment-vertical-"+D[1])}break}};var tabbedPanel={onCreate:function(e){
...
var i=$(this).parent().hasClass("static-text"),a=$(this).parent().hasClass("edit-text");if(i||a){var n=$(this).height(),d=!1;i&&25.5<n?d=!0:a&&22.5<n&&(d=!0),d?$(this).addClass("multiline"):$(this).removeClass("multiline")}
