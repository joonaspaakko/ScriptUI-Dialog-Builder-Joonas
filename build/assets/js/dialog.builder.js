...
function numberInputs(){function o(e,t,i,a){var n=e.val();if(isNaN(n))e.val(g);else{n=parseInt(n,10);var d=$("#dialog .active"),o=d.width(),s=e.hasClass("width"),r=d.height(),l=e.hasClass("height");0===n?e.removeClass("danger-zone"):s&&n<o?e.addClass("danger-zone"):s&&o<=n?e.removeClass("danger-zone"):l&&n<r?e.addClass("danger-zone"):l&&r<=n&&e.removeClass("danger-zone");var c={min:parseInt(e.attr("min"),10),max:parseInt(e.attr("max"),10),step:a||parseInt(e.attr("step"),10),modifierStep:parseInt(e.attr("modifier-step"),10)},p=(i?c.modifierStep:c.step)||1,m=n<c.min,f=n>c.max,u=m||f;switch(t){case"blur":if(u){var v=m&&c.min||f&&c.max;e.val(v)}break;case"up":n<c.max&&(n=n+p>c.max?c.max:n+p,e.val(n),g=n);break;case"down":n>c.min&&(n=n-p<c.min?c.min:n-p,e.val(n),g=n);break}}item.funnel.update(e.data("edit"))}$(".number").wrap('<div class="number-wrap">');var g,e=$(".number-wrap");$('<div class="arrow plus"></div><div class="arrow minus"></div><div class="number-overlay"></div>').appendTo(e);var t=e.find(".number"),i=e.find(".number-overlay");t.each(function(){"none"===$(this).css("display")&&$(this).closest(".number-wrap").addClass("hide")});var s,r,l=!1,c=0,p=null,m=null;$(window).on("mousedown mousemove mouseup",function(e){if("mousedown"===e.type)l=!0,s=$(e.target).parent(),r=s.find("> .number");else if("mousemove"===e.type){if(l&&mousemovePing&&r.hasClass("number")){
...
// TEXT
var v=item.list[a.toLowerCase()](!1).multiline;if(void 0!==s.text&&0<s.text.length)c+=p+r+'.text = "'+(v?s.text.split("\n").join("\\r"):s.text)+'"; \n';
// CHECKED
// PREFERRED SIZE
if(!0===s.checked&&(c+=p+r+".value = "+s.checked+"; \n"),void 0!==s.preferredSize&&"TreeView"!==a){var g=s.preferredSize[0],h=s.preferredSize[1];0<g&&(c+=p+r+".preferredSize.width = "+g+"; \n"),0<h&&(c+=p+r+".preferredSize.height = "+h+"; \n")}
// JUSTIFY
...
// MARGINS
case"margins":var p=t[0],m=t[1],f=t[2],u=t[3],v="object"!=typeof t,g=v?t:p,h=v?t:m,b=v?t:f,y=v?t:u;"Dialog"===i?o.css({paddingTop:g<=6?1:g,paddingRight:h<=1?1:h,paddingBottom:b<=1?1:b,paddingLeft:y<=1?1:y}):"Panel"===i||"Tab"===i?o.css({paddingTop:g<=3?3:g,paddingRight:h<=3?3:h,paddingBottom:b<=1?1:b,paddingLeft:y<=3?3:y}):o.css({paddingTop:g,paddingRight:h,paddingBottom:b,paddingLeft:y});break;
// PREFERRED SIZE
...
var d=t[0],o=t[1];i.find('#align-children-horizontal option:contains("'+d+'")').prop("selected",!0),i.find('#align-children-vertical option:contains("'+o+'")').prop("selected",!0);break;case"alignment":var s,r=$("#dialog .active").parent("div").parent("div").hasClass("orientation-column"),l=r?"column":"row",c=r?"horizontal":"vertical",p=null===t,m=r?["left","center","right","fill"]:["top","center","bottom","fill"],f="";$.each(m,function(e,t){f+='<option value="'+t+'">'+t+"</option>"}),i=$('<div class="alignment-container"><h2 title="The alignment style for this element. If defined, this value overrides the alignChildren setting for the parent container.">Alignment <span>(Self)</span></h2><div class="alignment-checkbox"><input type="checkbox" id="alignment-checkbox-input" name="" '+(p?"":"checked")+' /><label for="alignment-checkbox-input"></label></div><br>'+('<div id="alignment-'+c+'"><select name="qty" class="pretty-classic" data-edit="alignment"  data-edit-value="'+c+'" '+(p?"disabled":"")+" >"+f+"</select></div>")+"</div>"),"column"===l?(s=("top"===t?"left":"bottom"===t&&"right")||t,i.find('#alignment-horizontal option:contains("'+(null===t?"center":s)+'")').prop("selected",!0)):(s=("left"===t?"top":"right"===t&&"bottom")||t,i.find('#alignment-vertical option:contains("'+(null===t?"center":s)+'")').prop("selected",!0));break}return i}},propsPanel=$("#panel-edit-style-wrap");
...
$.each(data.order,function(e,t){void 0!==data.items["item-"+t].style.preferredSize&&(item.activate(t),item.funnel.update("preferredSize"))}),
...
var r=local_storage.get("dialog"),n=e.data("item-id"),l=$("#panel-tree-view-wrap"),c=Math.abs(n-item.get.id());
// Dragged item and every child with data-item-id attribute
e.find("[data-item-id]").add(e).each(function(e){var t=$(this).data("item-id"),i=r.items["item-"+t],
// active      = treeView.find('.active'),
// isParent    = active.data('parent'),
a=0===e?d.target.parent("li").data("item-id"):i.parentId+c,n={id:t+c,type:i.type,parentId:a,target:0===e?s:l.find('[data-item-id="'+a+'"] > ul'),event:0===e?"drag-duplicate":"loadFromLocalStorage",previousIsParent:o,sourceId:t};item.funnel.create(n)}),
// Reactivate the ye olde active item
item.activate(e.data("item-id")+c);
// Build Item Properties panel
var p=r.items["item-"+e.data("item-id")];edit_style_panel.build(p.style),$("body").removeClass("dragging")};var dialogElem=$("#dialog");
